...
// Initialize variables for brightness/overtemp
int currPot = 0;
int currTemp = 0;
...
void factoryReset(){
  tone(TEST_LED_PIN, 750, 2000);
}
void updateBrightnessPot(int desiredBrightness){
  ...
  /*if((temp1+temp2)/2>400){ //375 is placeholder value, need calibrated val
    pot -> increment();//pot -> set();
    //currPot--;
  } else {//((temp1+temp2)/2 <= 370) {
    if((pot -> get()) > desiredBrightness){ // can only increase brightness as long as we havent maxed out
      pot -> decrement();
      currPot++;
    }
  }
  /*} else {
  */
  pot -> set(desiredBrightness);
  //currPot = desiredBrightness;
  //}*/
}
...
void main_driver() {
  ...
  if (boosted == 2) {
    ...
    if (brightness == 1) { // light is bright
      updateBrightnessPot(pot_bright);
    } else if (brightness == 2) { // light is dim
      updateBrightnessPot(pot_dim);
    } else if (brightness == 4) { // light is off
      updateBrightnessPot(pot -> max_value()); 
