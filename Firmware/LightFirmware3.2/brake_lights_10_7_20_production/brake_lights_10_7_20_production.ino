...
MCP4XXX * pot;
int pot_bright = 24;//24 // pot value for max brightness (front light)
int pot_medium = 60; //60 pot value for max brightness (front light)
int pot_dim = 80 ;   //80 pot value for min brightness (front light)
...
void setup() {
  ...
  if((EEPROM.read(0)) == 0) { // if never writen (first time uploading)
    ...
    EEPROM.write(3, 2);//2nightmode
    EEPROM.write(4, 2);//3accel
