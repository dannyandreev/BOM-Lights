...
MCP4XXX * pot;
int pot_max = 50; // absolute max voltage (53.4 ~= 666mA) 
int pot_bright = 65; // pot value for max brightness (front light)
int pot_medium = 70;
int pot_dim = 90 ;   // pot value for min brightness (front light)

//Temp Settings
int temp_max = 307; // this is 100*C
int temp_high = 256; //this is 75*C
int temp_good =  153; // this is 25*C
// Set up button
...
const long accel_blink_interval = 750;
...
void loop() {
 ...
 /* float totI = 0;
  ...
  float totV = 0;
  for(int i = 0; i < 10; i++){
    //totI = totI + analogRead(6);
    //totT = totT + (analogRead(TEMP1_PIN)+analogRead(TEMP2_PIN))/2;
    float t1 =analogRead(TEMP1_PIN);
    float t2 = analogRead(TEMP2_PIN);
    float l1 = analogRead(LIGHT1_PIN);
    float l2 = analogRead(LIGHT1_PIN);
    Serial.print(t1*5/1024);
    Serial.print("   ");
    Serial.print(t2*5/1024);
    Serial.print("   ");
    Serial.print(l1*5/1024);
    Serial.print("   ");
    Serial.println(l2*5/1024);
    //totL = totL + (analogRead(LIGHT1_PIN)+analogRead(LIGHT2_PIN))/2;
    //totV = totV + analogRead(7);
    delay(1);
  }
  /*
  Serial.print("Iout: ");
  Serial.print(totI/10);
  Serial.print(" Temp: ");
  Serial.print(totT/10);
  Serial.print(" Lout: ");
  Serial.print(totL/10);
  Serial.print(" Vin: ");
  Serial.println(totV/10);
  */
...
void main_driver() {
  ...
  if (boosted == 2) {
    ...
    if (brightness == 1) { // light is bright
      ...
      setBrightnessPot(pot_medium);//pot -> set(pot_medium);
    } else if (brightness == 3) { // light is dim
      setBrightnessPot(pot_dim);//pot -> set(pot_dim);
    } 
...
void blink_indicator(int num) { //indicates what sub menu you are in.
  for (int i = 1; i <= num; i++) {
...
void calibrate() {
  //ready to calibrate light
  //while( button not pressed ) beep once, wait, beep again
  
  //calibrate temperature
  //while( button not pressed ) beep twice, wait, beep again
  float tempCal1 = 0;
  float tempCal2 = 0;
  for(int i = 0; i < 10; i++){
    delay(1);
    tempCal1 = tempCal1 + analogRead(TEMP2_PIN);
    tempCal2 = tempCal2 + analogRead(TEMP1_PIN);
  }
  if(tempCal1 > 1 || tempCal2 > 1){ // calibrating too hot should be at room temp (25*C) 
   calError(1);//temp cal error
  }
  EEPROM.write(8, tempCal1); // store low end temp for TEMP1_PIN
  EEPROM.write(10, tempCal2);// store low end temp for TEMP2_PIN
  //measure current temperature and set to room temp.
  //turn to abs max brightness and wait 30 sec. Set to uppre limit.
  
  
}
void calError(int errNum){
  if(errNum == 1){
    while(1){
      tone(TEST_LED_PIN, 1000, 200);
    }  
