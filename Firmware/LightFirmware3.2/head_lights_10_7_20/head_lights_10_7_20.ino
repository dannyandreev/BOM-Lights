...
MCP4XXX * pot;
int pot_bright = 72; // pot value for max brightness (front light)
...
void setup() {
  ...
  if((EEPROM.read(0)) == 0) { // if never writen (first time uploading)
    ...
    EEPROM.write(1, 1);//brightness
    EEPROM.write(2, 1);//nightmode
    EEPROM.write(3, 3);//accel
    EEPROM.write(4, 5);//rgb
    EEPROM.write(5, 1);//brake
    EEPROM.write(6, 1);//boosted
  }
  ...
  night = EEPROM.read(2);
  riding = EEPROM.read(3);
  rgb = EEPROM.read(4);
  brake = EEPROM.read(5);
  boosted = EEPROM.read(6);
