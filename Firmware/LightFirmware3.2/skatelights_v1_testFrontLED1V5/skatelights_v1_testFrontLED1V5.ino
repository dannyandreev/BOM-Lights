...
MCP4XXX * pot;
int pot_bright = 50; //dimmed for test // 50ish pot value for max brightness (front light)
int pot_dim = 80 ;   // pot value for min brightness (front light)
int pot_test = 127;
// Set up button
...
unsigned int brightness_length = 4;
...
void loop() {
  ...
  digitalWrite(RED_TEST_LED, HIGH);
  digitalWrite(GREEN_TEST_LED, HIGH);
  Serial.write("RED, Green; HIGH");
  Serial.println();
  delay(500);
  
  digitalWrite(RED_TEST_LED, LOW);
  ...
  Serial.println();
  */
  tone(TEST_LED_PIN, (temp1+temp2), 500);
  delay(500);
  
  int light1 = analogRead(LIGHT1_PIN);
  int light2 = analogRead(LIGHT2_PIN);
  /*
  Serial.write("Light 1: ");
  Serial.print(light1);
  Serial.println();
  Serial.write("Light 2: ");
  Serial.print(light2);
  Serial.println();
  */
  tone(TEST_LED_PIN, (light1+light2)/2, 500);
  delay(500);
  noTone(TEST_LED_PIN);
  /*
  
  analogRead(LIGHT1_PIN);
...
void main_driver() {
  ...
  if (boosted == 2) {
    ...
    if (brightness == 1) { // light is bright
      pot -> set(pot_bright);
    } else if (brightness == 2) { // light is dim
      pot -> set(pot_dim);
    } else if (brightness == 4) { // light is off
      pot -> set(pot -> max_value()); 
    } else if (brightness == 3 && brake != 2 && brake != 3) { //REWRITE // the lights should blink and ignore break status
  ...
  if (mcp2515.readMessage( & frame) == MCP2515::ERROR_OK) {
    ...
    if (frame.can_dlc == 8 && frame.data[1] == 0x04) {
      ...
      if (frame.data[2] == 0x22 && frame.data[0] == 0x1) {
        ...
        // Front LED is always constant independent of braking
        //        digitalWrite(POWER_12V_CONTROL_PIN, HIGH);

        // This indicates braking
        if (frame.data[3] == 0x64)

        {
          can_current_millis = millis();
          ...
          can_blink_timer = can_blink_timer + can_current_millis - can_previous_millis;

          if (can_blink_timer > can_blink_interval) {
            can_blink_timer = 0;
            can_blink = !can_blink;
          }
          can_previous_millis = can_current_millis;
      ...
      else if (frame.data[2] == 0x22 && frame.data[0] == 0x1) {
        digitalWrite(LED_CONTROL_PIN, LOW);
        digitalWrite(POWER_12V_CONTROL_PIN, LOW);
      }
    }
